          ...
          <div className="grid lg:grid-cols-2 gap-14 items-center">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
              ...
              <div className="flex gap-4 flex-wrap">
                <Link href="/courses" className="glow-btn px-6 py-3 rounded-xl font-semibold text-white flex items-center gap-2">
                  Explore Courses <ArrowRight size={16} />
                </Link>
                <Link href="/it-services" className="outline-btn px-6 py-3 rounded-xl font-medium">
                  Our Services
                </Link>
              </div>
            </motion.div>
            ...
            <motion.div initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.2 }}
              ...
              {[
                ...
                <div key={i} className="bg-white rounded-2xl p-6 text-center card-hover border border-gray-100"
                  style={{ boxShadow: "0 2px 16px rgba(0,0,0,0.05)" }}>
                  <div className="w-12 h-12 rounded-xl flex items-center justify-center mx-auto mb-3"
                    style={{ background: s.color }}>
                    <s.icon size={20} className="text-white" />
                  </div>
                  <div className="number-stat text-3xl font-extrabold mb-1">{s.value}</div>
          ...
          <div className="grid md:grid-cols-2 gap-8 max-w-4xl mx-auto">
            {[
              ...
              <motion.div key={item.title}
                ...
                <h2 className="font-bold text-2xl text-gray-900 mb-4">{item.title}</h2>
              ...
              {milestones.map((m, i) => (
                <motion.div key={m.year}
                  ...
                  <div className="absolute left-0 w-12 h-12 rounded-xl bg-blue-50 border border-blue-200 flex items-center justify-center">
                    <span className="text-blue-600 text-xs font-bold">{m.year.slice(2)}</span>
                  </div>
                  <div className="bg-white rounded-2xl p-5 flex-1 border border-gray-100"
